...
class Sign_up:
    ...
    def __init__(self, signup_page):
        ...
        side_image = Image.open(
            r"C:\Users\Amith\Desktop\New folder\Photo\img2.jpg")
        ...
        # log-in:
        login_button = Button(self.signup_page, text="Log-in", font=("times new roman", 15, "bold",
                              "underline"), foreground="#6007cb", cursor="hand2", relief="flat", command=self.Login)
        login_button.place(x=1420, y=0, width=85, height=40)

        # frame for Sing-Up

        signup_frame = LabelFrame(self.signup_page, bd=1, relief="flat", text="Join our Community", font=(
            "times new roman", 15, "bold", "underline"), background="white", foreground="#6007cb")
        ...
        firstname = Label(signup_frame, text="First Name", font=(
            "times new roman", 15), anchor="w", background="white")
        firstname.place(x=20, y=10, width=100, height=15)

        firstname_textbox = Entry(signup_frame, textvariable=self.var_firstname, font=(
            "times new roman", 15), bd=2)
        firstname_textbox.place(x=20, y=40, width=170, height=30)

        lastname = Label(signup_frame, text="Last Name", font=(
            "times new roman", 15), anchor="w", background="white")
        lastname.place(x=300, y=10, width=100, height=15)

        lastname_textbox = Entry(signup_frame, textvariable=self.var_lastname, font=(
            "times new roman", 15), bd=2)
        lastname_textbox.place(x=300, y=40, width=170, height=30)

        Email_id = Label(signup_frame, text="Email", font=(
            "times new roman", 15), anchor="w", background="white")
        Email_id.place(x=20, y=110, width=100, height=15)

        email_id_textbox = Entry(
            signup_frame, textvariable=self.var_email, font=("times new roman", 15), bd=2)
        email_id_textbox.place(x=20, y=140, width=250, height=30)

        location = Label(signup_frame, text="Location", font=(
            "times new roman", 15), anchor="w", background="white")
        location.place(x=20, y=210, width=100, height=15)

        loc_combobox = ttk.Combobox(signup_frame, textvariable=self.var_location, font=(
            "times new roman", 15), width=20)
        loc_combobox["values"] = (
            "Enter Location", "Bangalore", "Delhi", "Kashmir", "Pune", "Belgaum")
        loc_combobox.current(0)
        loc_combobox.place(x=20, y=240, width=170, height=30)

        Password = Label(signup_frame, text="Password", font=(
            "times new roman", 15), anchor="w", background="white")
        Password.place(x=20, y=310, width=100, height=15)

        password_textbox = Entry(signup_frame, textvariable=self.var_password, font=(
            "times new roman", 15), bd=2)
        password_textbox.place(x=20, y=335, width=250, height=30)

        # SignUp:
        signup_button = Button(signup_frame, text="Sign-Up", font=("times new roman", 15, "bold"),
                               background="#6007cb", cursor="hand2", relief="flat", foreground="white", command=self.error_message)
    ...
    def error_message(self):
        if self.var_firstname.get() == "" and self.var_lastname.get() == "" and self.var_email.get() == "" and self.var_location.get() == "Enter Location" and self.var_password.get() == "":
            ...
            messagebox.showwarning(
                "Warning", "Please enter the First name", parent=self.signup_page)
        elif self.var_lastname.get() == "":
            messagebox.showwarning(
                "Warning", "Please enter the Last name", parent=self.signup_page)
        elif self.var_email.get() == "":
            messagebox.showwarning(
                "Warning", "Please enter the Email id", parent=self.signup_page)
        elif self.var_location.get() == "Enter Location":
            messagebox.showwarning(
                "Warning", "Please enter the location name", parent=self.signup_page)
        elif self.var_password.get() == "":
            messagebox.showwarning(
                "Warning", "Please enter the password name", parent=self.signup_page)
        else:
            pass
